 ...
 * @param {*} redisConfig
 */
async function redisClient(redisConfig) {
  try {
    let setting = {
      host: redisConfig.host,
      port: redisConfig.port
    }
    let redisClient = Promise.promisify(redis.createClient(setting))
    if(undefined != redisConfig.password) await redisClient.authAsync(redisConfig.password)
    return redisClient
  } catch (e) {
    throw new Error(e)
  }
